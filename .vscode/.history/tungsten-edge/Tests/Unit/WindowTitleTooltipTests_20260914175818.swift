...
final class WindowTitleTooltipTests: XCTestCase {
    ...
    func testInStripTitleWidthTracksTheDockScale() {
        ...
        let baseline = WindowTitleTextMetrics.defaultMaximumWidth
        for tier in DockSize.allCases {
            XCTAssertEqual(WindowTitleTextMetrics.maximumWidth(baseline, for: tier.scale),
                           baseline * tier.scale, accuracy: 0.001)
        }
        // 中档必须与历史字面值一致。
        XCTAssertEqual(WindowTitleTextMetrics.maximumWidth(baseline, for: 1.0), 140)
        // 用户调过的基线同样按档位缩放：上限现在是可配置的，不再写死 140。
        XCTAssertEqual(WindowTitleTextMetrics.maximumWidth(90, for: 1.0), 90)
        XCTAssertEqual(WindowTitleTextMetrics.maximumWidth(200, for: 2.0), 400)
    }

    /// 出厂默认基线是历史实测的中档 140，且和 `AppSettingsStore` 那份各写一份的字面值相等
    /// （Composition 层不引 Scenes 层的度量类型，用这条测试锁住两处不分叉）。
    func testDefaultMaximumWidthMatchesTheStoreDefault() {
        XCTAssertEqual(WindowTitleTextMetrics.defaultMaximumWidth, 140)
        XCTAssertEqual(AppSettingsStore.defaultWindowTitleMaxWidth,
                       WindowTitleTextMetrics.defaultMaximumWidth)
    }
