...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    /// 「窗口标题宽度 ▸」：子菜单里放一条即时生效的滑块。做成子菜单同「Dock 栏大小 ▸」——
    /// 主菜单只多一行、高度恒定，滑块的含义由父项标题给出（那一块没有组帽子，不便再摆裸滑块）。
    private let titleWidthItem = NSMenuItem(title: String(localized: "Window title width"), action: nil, keyEquivalent: "")
    private let titleWidthMenu = NSMenu()
    private let titleWidthSliderView: TitleWidthSliderMenuItemView
    ...
    init(store: AppSettingsStore,
        ...
        titleWidthSliderView = TitleWidthSliderMenuItemView(accessibilityTitle: String(localized: "Window title width"))
        super.init()
        configureStatusItem()
        configureMenu()
        refreshCheckmarks()
