...
struct ChipView: View {
    ...
    let scale: CGFloat
    /// 条内标题最大宽度的中档基线（`AppSettingsStore.windowTitleMaxWidth`，用户可在菜单里调）。
    /// **故意不给默认值**——漏传会让这张卡按写死的 140 排版、和别处调过的宽度对不上，
    /// 同 `scale`：必须是编译错误。渲染的 `Text`、药丸宽、气泡锚点三处共用它。
    ...
    private var multiWindowChip: some View {
        ...
        .chipQuietHoverScale(
            quietHoverFeedback,
            cardWidth: ChipPillMetrics.width(title: capturedDisplayTitle, maxTitleWidth: maxTitleWidth, scale: scale)
