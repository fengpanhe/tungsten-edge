...
struct DockStripView: View {
    ...
    /// 条内窗口标题最大宽度的中档基线（用户可在状态栏菜单里调）。每张带标题卡显式接收它
    /// （同 `dockScale`，漏传是编译错误）；变宽变窄由 `PanelCoordinator` 订阅 store 后 relayout。
    private var maxTitleWidth: CGFloat { settingsStore.windowTitleMaxWidth }
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                ...
                .onChange(of: projection.labelTitleByChipID) { newTitles in
                    ...
                    for id in titled {
                        ...
                        let from = ChipPillMetrics.labelWidth(title: old, maxTitleWidth: maxTitleWidth, scale: dockScale)
                        if from != ChipPillMetrics.labelWidth(title: new, maxTitleWidth: maxTitleWidth, scale: dockScale) { starting[id] = from }
                    }
